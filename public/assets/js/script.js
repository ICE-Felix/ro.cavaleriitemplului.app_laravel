...
const collapse = () => {
  ...
  const toggleCollapse = (collapseTrigger) => {
    ...
    if (accordion) {
      ...
      const accordions = accordion.querySelectorAll(".collapsible");
...
const tabs = () => {
  ...
  on("body", "click", '[data-toggle="tab"]', (event) => {
    ...
    const activeTab = tabs.querySelector(".collapsible.open");
...
const toasts = () => {
  ...
  const createToast = (toast) => {
    ...
    // Auto dismiss
    if (toast.dataset.dismiss) {
      setTimeout(() => closeToast(newToast), toast.dataset.dismiss);
    }
  };
